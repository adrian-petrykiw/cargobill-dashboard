...
export const verificationCodeRepository = {
   ...
   */
  async markAsUsed(supabase: SupabaseClient<Database>, id: string): Promise<void> {
    try {
  ...
  /**
   * Cleans up expired verification codes - System operation
  ...
  // System operations for initialization and management
  async getByCodeSystem(code: string, purpose: string): Promise<VerificationCode | null> {
    try {
      const { data, error } = await supabaseAdmin
        .from('verification_codes')
        .select('*')
      ...
      logger.error({
        message: 'Exception in getByCodeSystem',
        purpose,
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
  ...
  async createSystem(
    ...
    try {
      ...
      logger.error({
        message: 'Exception in createSystem verification code',
        purpose,
        userId,
        email,
        error: error instanceof Error ? error.message : String(error),
