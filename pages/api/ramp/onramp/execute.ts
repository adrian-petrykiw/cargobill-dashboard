// pages/api/ramp/onramp/execute.ts
...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (txError || !txData) {
      return res.status(404).json({
        ...
        error: {
          ...
          message: 'Simulation data not found',
        },
      });
    }
    ...
    if (!executeResult || !executeResult.success) {
      console.error('Failed to execute transfer:', executeResult);
      return res.status(500).json({
        success: false,
        error: {
          code: 'EXECUTION_FAILED',
          message: 'Failed to execute transfer with Zynk',
